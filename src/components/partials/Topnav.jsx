      ...
      <div className="relative left-1/2 -translate-x-1/2 sm:w-[35vw] w-[80vw] flex items-center gap-5">
        ...
        <div className="w-[100%] max-h-[30vh] sm:max-h-[20vw] absolute top-[100%] left-0 overflow-x-hidden overflow-y-auto mb-2 bg-zinc-800 px-2 z-10">
          {search&& search.map((item, index) => (
            <Link to={`/${item.media_type}/details/${item.id}`} key={index} className="w-full h-[10vh] sm:h-[10vw]  flex items-end sm:items-center justify-between mb-6 sm:mb-3 hover:bg-zinc-700 ">
              <div className="h-full w-[30%]  rounded-xl overflow-hidden">
                <img className=" h-full w-full object-cover" src={item.backdrop_path || item.poster_path ? `https://image.tmdb.org/t/p/original/${item.backdrop_path || item.poster_path}` : `https://www.shoshinsha-design.com/wp-content/uploads/2020/05/noimage_icon-1.png`} alt="" />
              </div>
              <div className="h-full w-[70%] font-bold px-7 mt-7">
                <h1 className="text-xl sm:text-2xl">{item.name || item.title || item.original_name || item.original_title}</h1>
              </div>
            </Link>
          ))}
        </div>
